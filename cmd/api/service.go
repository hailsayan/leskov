...
func (s *Server) createOrder(products []types.Product, cartItems []types.CartCheckoutItem, userID int) (int, float64, error) {
	...
	for _, item := range cartItems {
		...
		s.store.Product.UpdateProduct(product)
	}

	// create order record
	orderID, err := s.store.Order.CreateOrder(types.Order{
		UserID:  userID,
		Total:   totalPrice,
		Status:  "pending",
	...
	for _, item := range cartItems {
		s.store.Order.CreateOrderItem(types.OrderItem{
			OrderID:   orderID,
